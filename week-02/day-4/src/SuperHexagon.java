...
public class SuperHexagon {
  ...
  private static int[][] createHexagonCoordinates(int x0, int y0, int borderLength,
      Graphics graphics) {
        /*
    ...
    for (int i = 0; i < ySteppes.length; i++) {
      if (i == 0) {
        ...
        hexagonCorners[0][i] = (int) ((double) hexagonCorners[0][i - 1] + (borderLength
            * xSteppes[i]));
        hexagonCorners[1][i] = (int) ((double) hexagonCorners[1][i - 1] + (borderLength
            * ySteppes[i]));
      }
    }
